...
import { PG_PROGRAMMES_GROUPED, getProgrammeDisplay } from "../utils/pgProgrammes";
...
const IITG_HOSTELS = [
  "Kameng", "Barak", "Disang", "Umiam", "Gaurang", "Manas",
  "Subhansiri", "Brahmaputra", "Dihing", "Siang", "Lohit", "Kapili", "Dhansiri",
];
...
function DeadlineBanner({ deadline }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setExpired(true); setTimeLeft("Deadline has passed"); return; }
  ...
  return (
    <p className={`text-right text-xs md:text-sm mb-8 ${expired ? "text-red-400" : "text-amber-400"}`}>
      {deadline.title}:{" "}
      <span className="font-semibold">{new Date(deadline.deadline_date).toLocaleString()}</span>
      {" "}&nbsp;|&nbsp;
    ...
    <div className="campus-bg min-h-screen w-full flex items-center justify-center p-4" style={campusBgStyle}>
      <div className="glass-card w-full max-w-4xl rounded-2xl p-8 md:p-12 text-slate-900">
        {deadline ? <DeadlineBanner deadline={deadline} /> : (
          <p className="text-right text-xs md:text-sm text-slate-500 mb-8">No submission deadline set yet</p>
        )}

        <div className="flex justify-center mb-6">
          <img src={`${process.env.PUBLIC_URL}/iitg.png`} alt="IITG" className="h-20 w-20 md:h-24 md:w-24 object-contain" />
        </div>

        <h1 className="text-center text-3xl md:text-4xl font-bold text-slate-900">Placement Portal</h1>
        <h2 className="text-center text-xl md:text-2xl font-semibold text-[var(--brand)] mt-1">Photo Collection</h2>
        <p className="text-center text-slate-600 mt-3 text-sm md:text-base">Centre for Career Development, IIT Guwahati</p>
        ...
        <p className="mt-4 text-center text-xs text-slate-400">Only registered PG students can access this portal</p>
      </div>
    </div>
  );
}
    ...
    <div className="campus-bg min-h-screen w-full flex items-center justify-center p-4" style={campusBgStyle}>
      <div className="glass-card w-full max-w-3xl rounded-2xl p-8 text-slate-900">
        <div className="flex items-center justify-end gap-3 mb-4 text-sm">
          ...
          <button type="button" onClick={onLogout} className="rounded-md bg-slate-200 px-3 py-1.5 font-semibold hover:bg-slate-300">Logout</button>
        </div>
        ...
        <p className="mt-2 text-center text-sm text-slate-600">
          {canEdit ? "You can still update your photo before the deadline." : "The deadline has passed. You can view your submitted photo."}
        </p>
        ...
        <div className="mt-5 mx-auto max-w-sm rounded-xl border border-slate-200 bg-white/70 px-5 py-4 text-sm space-y-1.5">
          ...
          <div><span className="font-medium">Hostel:</span> {profile?.hostel || "—"}</div>
          <div><span className="font-medium">Programme:</span> {getProgrammeDisplay(profile?.programme)}</div>
        </div>

        {canEdit && (
          <div className="mt-8 flex justify-center">
            <button type="button" onClick={onEdit} className="rounded-md bg-[var(--brand)] px-6 py-2 text-white font-semibold hover:opacity-90 transition">
...
function RegistrationForm({ profile, deadline, onLogout, onSubmitted }) {
  ...
  const [form, setForm] = useState({
    ...
    programme: profile?.programme ? String(profile.programme) : "",
  });
  ...
  const handlePhotoChange = (e) => {
    ...
    if (!["image/jpeg", "image/jpg", "image/png"].includes(file.type)) {
      toast.error("Only JPEG and PNG images are allowed"); e.target.value = ""; return;
    }
    if (file.size > 5 * 1024 * 1024) {
      toast.error("Photo must be smaller than 5 MB"); e.target.value = ""; return;
    }
    setPhotoFile(file);
  ...
  const handleSaveAndUpload = async () => {
    ...
    if (!form.programme) { toast.error("Please select your programme"); return; }
    ...
    <div className="campus-bg min-h-screen w-full flex items-center justify-center p-2 sm:p-3 md:p-4" style={campusBgStyle}>
      <div className="glass-card w-full sm:max-w-3xl md:max-w-4xl rounded-2xl p-4 md:p-6 text-slate-900 max-h-[calc(100vh-1rem)] md:max-h-[88vh] overflow-y-auto overflow-x-hidden hide-scrollbar">
        ...
        <div className="mb-5">
          ...
          <p className="text-sm text-slate-500 mt-1">Fill your details and upload your photo for the Placement ID card.</p>
        </div>
        ...
        {/* Editable fields — 2-col grid like intern form */}
        <div className="grid md:grid-cols-2 gap-3 text-sm mb-5">
          ...
          <label className="flex flex-col gap-1">
            <span>Hostel</span>
            <select
              className="rounded border border-slate-300 px-3 py-2 bg-white disabled:bg-slate-50 disabled:text-slate-400"
              value={form.hostel}
          ...
          <label className="flex flex-col gap-1 md:col-span-2">
            <span>Programme <span className="text-red-500">*</span></span>
            <select
              className="rounded border border-slate-300 px-3 py-2 bg-white disabled:bg-slate-50 disabled:text-slate-400"
            ...
            >
              ...
              {PG_PROGRAMMES_GROUPED.map(({ group, programmes }) => (
        ...
        {status && (
          <p className={`mt-2 text-sm ${status.toLowerCase().includes("success") ? "text-emerald-600" : "text-red-600"}`}>{status}</p>
        )}
...
export default function PgRegistrationPage() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      if (res.status === 401 || res.status === 403) { clearSession(); navigate("/", { replace: true }); return; }
      const data = await res.json();
      setProfile(data);
    } catch {
  ...
  const handleLogin = () => { setLoginLoading(true); window.location.href = `${API_BASE}/auth/outlook/login`; };
  ...
  if (!isLoggedIn) return <WelcomeScreen deadline={deadline} onLogin={handleLogin} loading={loginLoading} />;
  ...
  return (
    <RegistrationForm
      ...
      deadline={deadline}
      onLogout={handleLogout}
      onSubmitted={(updated) => { setProfile(updated); setShowForm(false); }}
    />
  );
}
